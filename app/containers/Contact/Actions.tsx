...
export const LeftActions = React.memo(({ transX, width, onToggleFav, favorite, displayMode }: ILeftActionsProps) => {
		...
		<View style={[styles.actionsContainer, styles.actionsLeftContainer]} pointerEvents='box-none'>
			...
			>
				<View style={[styles.actionLeftButtonContainer, viewHeight]}>
					<RectButton style={styles.actionButton} onPress={onToggleFav}>
						<CustomIcon
...
export const RightActions = React.memo(({ transX, width, onEdit, displayMode }: IRightActionsProps) => {
	const { colors } = useTheme();
	...
	return (
		<View style={[styles.actionsLeftContainer, viewHeight]} pointerEvents='box-none'>
			...
			>
				<RectButton style={[styles.actionButton, { backgroundColor: colors.hideBackground }]} onPress={onEdit}>
					<CustomIcon size={isCondensed ? CONDENSED_ICON_SIZE : EXPANDED_ICON_SIZE} name='edit' color={colors.buttonText} />
				</RectButton>
			</Animated.View>
		</View>
	);
